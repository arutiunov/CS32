...
int firstFalse(const string a[], int n)
{
    ...
    int placeholder = firstFalse(a, n - 1);
    
    if (!somePredicate(a[n - 1]) && placeholder == -1)
    {
        return n - 1;
    }
...
{
    if (n2 == 0) // reached end of smaller array (all elements found)
        return true;
    if (n1 == 0) // reached end of larger array (not all elements found)
        return false;
    
    if (a2[0] != a1[0]) // array elements not the same
    {
        return includes(a1 + 1, n1 - 1, a2, n2); // shift larger array over
    }
    else // array elements the same
        return includes(a1 + 1, n1 - 1, a2 + 1, n2 - 1); // shift both arrays over
}
...
{
    {
        ...
        if (a1[0] == a2[0])
        {
            int m = countIncludes(a1 + 1, n1 - 1, a2 + 1, n2 - 1); // continue checking if this is the match
            int n = countIncludes(a1 + 1, n1 - 1, a2, n2); // skip this match and look for other matches
            return (m + n);
        }
        
        else // no  match, keep looking
        {
            int k = countIncludes(a1 + 1, n1 - 1, a2, n2);
            return k;
        }
    }
}
