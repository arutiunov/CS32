...
string createDeltaHelper(const string& oldString, const string& newString, int STRINGLENGTH);
...
string createDeltaHelper(const string& oldFileString, const string& newFileString, int STRINGSIZE)
{
    ...
    string deltaCommand; // string that we will ultimately return
    ...
    while (j < newFileString.size())
    {
        ...
        if (hashTable.buckets[index].size() != 0) // check that list is not empty (hash for string was found)
        {
            for (list<segment>::const_iterator p = hashTable.buckets[index].begin(); p != hashTable.buckets[index].end(); p++)
            {
                ...
                {
                    if (tempACommands.size() != 0) // add all the previous unfound chars to the delta command string
                    {
                        deltaCommand += "A" + to_string(tempACommands.size()) + ":";
                        for (int i = 0; i < tempACommands.size(); i++)
                            deltaCommand += tempACommands[i];
                    }
                    ...
                    while (newFileString[contNewIndex] == oldFileString[contOldIndex]) // comparing further match
                    {
                        ...
                        contNewIndex++, contOldIndex++;
                    }
                    
                    cCommandLength = contOldIndex - cCommandOffset; // ultimate length of command to feed into delta file (if no further match then it's just STRINGSIZE)
                    
                    if (cCommandLength + cCommandOffset <= oldFileString.size())
                    {
                        deltaCommand += "C" + to_string(cCommandLength) + "," + to_string(cCommandOffset); // insert command into delta file
                        matchFound = true;
...
return deltaCommand; // return the finals delta string generated by helper function
}


void createDelta(istream& oldf, istream& newf, ostream& deltaf)
{
    vector<string> deltaFiles; // create a vector that will store all the different deltas generated by helper function
    
    stringstream oldBuffer, newBuffer;
    oldBuffer << oldf.rdbuf();
    string oldFileString(oldBuffer.str()); // read in the old string
    newBuffer << newf.rdbuf();
    string newFileString(newBuffer.str()); // read in the new string
    
    int i = 2;
    while (i <= 50)
    {
        deltaFiles.push_back(createDeltaHelper(oldFileString, newFileString, i));
        i = i + 2; // N size increments of 2
    }
    
    string compare = deltaFiles[0];
    
    for (int i = 0; i < deltaFiles.size(); i++)
    {
        if (compare.size() > deltaFiles[i].size())
            compare = deltaFiles[i];
    }
    deltaf << compare;
}